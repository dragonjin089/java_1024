...
public class MethodBaseballGameEx01 {
	...
	//아웃
	public static int out(int []createNumber , int []user) 
	{
		int countNumber=(createNumber,user);
		{
			if(!contains(int tmp,arr)) {
				return 0;
			}
	...
	// 볼 
	public static int ball(int []createNumber , int []user) 
	{
		int countNumber=(createNumber,user);
		...
		for(int i=0;i<size;i++) 
		{
			...
			{
				if(createNumber[i]==user[j])
				{
					countNumber++;	
				}
				else if (createNumber[i]==user[i]) {
					countNumber--;
				}
			}
	...
	// 스트라이크 
	public static int strike(int []createNumber , int []user) 
	{
		int countNumber=(createNumber, user);
	...
	public static int countNumber(int []arr1, int []arr2) 
	{
