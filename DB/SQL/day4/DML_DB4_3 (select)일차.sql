...
속성|식 등
[from 테이블명];
[where 조건절 ] ;-- 조건에 맞는 결과를 조회 ==가 아닌 =을 이용하여 비교 , 같지 않은 경우는 != 같지 않은 경우는 !=를 이용
[order by 속성명[asc | desc}. 속성명2 [asc | desc]] ; -- 속성명 1로 정렬(오름차순 | 내림차순) 한 후 속성명 1의 값이 같은 행동을 속성명 2로 정렬(오름 | 내림)
-- 정렬방법이 생략되면 asc (오름차순) 
[limit 절]; 
-- 검색결과의 개수를 제한  
...
select me_id, me_pw
    ...
    -- member 테이블에 아이디가 abc 인 회원 정보를 조회 
 ...
 select * from member order by me_id limit 0,2;
  select * from member order by me_id limit 2;
   ...
   -- n번지 x 명 조회 limit (n-1) * x , x
   
   -- 게시글에서 조회수가 1 이상 100 이하인 게시글들을 조회 
   ...
   -- 게시글에서 카테고리번호가 1번 또는 2번인 게시글 전체를 조회 
   ...
   /* 문자열 검색
   -  =는 두 문자열이 동일해야 true 
   - like을 이용하여 문자열1이 문자열 2에 있는지 확인할 수 있다.
   ... where 속성명 like 문자열;
   - _ : 한 글자를 의미 
...
(select bc_num from board_category where bc_name ='공지사항');

-- 가전제품중(제품 카테고리 1번)  가장 비싼제품보다 가격이 높은 제품들
...
-- 모든회원의 권한을 출력(권한만 출력하기 때문에 중복이가능)
select me_authority from member;
-- distinct 중복제거 
select distinct me_authority from member;

-- A as B : A의 이름을 B로 수정(일시적) - select 문으로 검색 결과를 출력할 떄 
