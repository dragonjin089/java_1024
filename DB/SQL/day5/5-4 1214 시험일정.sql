/*
시험 
insert : 디비 사용자가 컴퓨터 db에 데이터를 추가하기 위해 사용하는 언어
select : db에 등록된 데이터를 조회하기 위해 사용하는 언어
update : db에 등록된 데이터를 수정하기 위해 사용하는언어
delete : db에 등록된 데이터를 삭제하기 위해 사용하는 언어

create : DB 객체를 생성(DB, 테이블, 사용자등) 할때 사용하는 언어 
...
대리키 : 테이블의 기본키가 복합 속성으로 되어 있는 경우 일련번호를 대체하는 속성을 기본키로 설정 
대체키 : 후보키중 기본키로 선정되지 못한 키 
