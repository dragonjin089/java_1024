...
$(function(){
	...
	let val = $(this).val();
		// 전역 배열에 val이 있는지 확인 또는 클릭한 요소의 checked여부를 확인 
		let index = tmpMenuArr.indexOf(val);
		// 배열에 val가 있으면 (또는 클릭한 요소가 checked가 해제되면)
		if(index >= 0){
			// 배열에 val를 제거 
			tmpMenuArr.splice(index,1);	
		}
			 
			// 없으면 
			else {
				// 배열의 길이가 4 이상이면 현재 선택한요소를 checked를 해제하고 알림창을 띄운 후 종료 
				if(tmpMenuArr.length >=4){
					$(this).prop('checked',false);
					alert('최대4개까지 설정할 수 있습니다.');
		 ...
		 // 주어진 전역 배열을 기준은로 list-empty-box 요소를 배치 (함수로 따로 만듬)
		...
		$('.group-menu .btn-save').click(function(){
			...
			selectNewsList = tmpMenuArr.splice(0);
			init();
...
function drawEmptyboxMenu(tmpMenuArr){
	...
	//체크박스 관리
	$('.group-service-check[type=checkvox]').each(function(){
		let val = $(this).val();
...
let tmpMenuArr =[];//선택한 메뉴를 저장할 임시 배열 (저장)
let selectedMenuArr = []; // 선택한 메뉴를 저장할 배열(저장완료)
